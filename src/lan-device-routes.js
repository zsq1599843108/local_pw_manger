...
//
// Endpoints:
//   POST /api/lan/devices/trust          { fingerprint, pubkey_b64, label }
//   GET  /api/lan/devices
...
function installLanDeviceRoutes(app, db) {
  app.post('/api/lan/devices/trust', (req, res) => {
    const { fingerprint, pubkey_b64, label } = req.body || {};
    ...
    try {
      pairedDevices.trustDevice(db, { fingerprint, label: safeLabel, pubkey });
    } catch (e) {
...
module.exports = {
  ...
  _internal: { looksLikeFingerprint, decodePubkey },
};
